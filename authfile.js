...
sjcl.bitArray = {
    bitSlice: function (a, b, c) { a = sjcl.bitArray.m(a.slice(b / 32), 32 - (b & 31)).slice(1); return void 0 === c ? a : sjcl.bitArray.clamp(a, c - b) }, extract: function (a, b, c) { var d = Math.floor(-b - c & 31); return ((b + c - 1 ^ b) & -32 ? a[b / 32 | 0] << 32 - d ^ a[b / 32 + 1 | 0] >>> d : a[b / 32 | 0] >>> d) & (1 << c) - 1 }, concat: function (a, b) { if (0 === a.length || 0 === b.length) return a.concat(b); var c = a[a.length - 1], d = sjcl.bitArray.getPartial(c); return 32 === d ? a.concat(b) : sjcl.bitArray.m(b, d, c | 0, a.slice(0, a.length - 1)) }, bitLength: function (a) {
...
sjcl.codec.base64 = {
    i: "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/", fromBits: function (a, b, c) { var d = "", e = 0, f = sjcl.codec.base64.i, h = 0, g = sjcl.bitArray.bitLength(a); c && (f = f.substr(0, 62) + "-_"); for (c = 0; 6 * d.length < g;)d += f.charAt((h ^ a[c] >>> e) >>> 26), 6 > e ? (h = a[c] << 6 - e, e += 26, c++) : (h <<= 6, e -= 6); for (; d.length & 3 && !b;)d += "="; return d }, toBits: function (a, b) {
        a = a.replace(/\s|=/g, ""); var c = [], d, e = 0, f = sjcl.codec.base64.i, h = 0, g; b && (f = f.substr(0, 62) + "-_"); for (d = 0; d < a.length; d++) {
            g = f.indexOf(a.charAt(d));
            if (0 > g) throw new sjcl.exception.invalid("this isn't base64!"); 26 < e ? (e -= 26, c.push(h ^ g >>> e), h = g << 32 - e) : (e += 6, h ^= g << 32 - e)
        } e & 56 && c.push(sjcl.bitArray.partial(e & 56, h, 1)); return c
    }
}; sjcl.codec.base64url = { fromBits: function (a) { return sjcl.codec.base64.fromBits(a, 1, 1) }, toBits: function (a) { return sjcl.codec.base64.toBits(a, 1) } }; sjcl.hash.sha256 = function (a) { this.g[0] || r(this); a ? (this.f = a.f.slice(0), this.c = a.c.slice(0), this.a = a.a) : this.reset() }; sjcl.hash.sha256.hash = function (a) { return (new sjcl.hash.sha256).update(a).finalize() };
sjcl.hash.sha256.prototype = {
    blockSize: 512, reset: function () { this.f = this.l.slice(0); this.c = []; this.a = 0; return this }, update: function (a) {
        "string" === typeof a && (a = sjcl.codec.utf8String.toBits(a)); var b, c = this.c = sjcl.bitArray.concat(this.c, a); b = this.a; a = this.a = b + sjcl.bitArray.bitLength(a); if (0x1fffffffffffff < a) throw new sjcl.exception.invalid("Cannot hash more than 2^53 - 1 bits"); if ("undefined" !== typeof Uint32Array) {
    ...
    }, finalize: function () { var a, b = this.c, c = this.f, b = sjcl.bitArray.concat(b, [sjcl.bitArray.partial(1, 1)]); for (a = b.length + 2; a & 15; a++)b.push(0); b.push(Math.floor(this.a / 0x100000000)); for (b.push(this.a | 0); b.length;)u(this, b.splice(0, 16)); this.reset(); return c }, l: [], g: []
};
function u(a, b) {
...
function createusernamebox() {
    ...
    txtbox.addEventListener('keydown', function (e) {
        if (13 === e.keyCode || e.keyCode === 9) {
            ...
            let unamehash = ForAuthentication(unameclr);
...
function showButton() {
    ...
    button.style.backgroundImage = "url(" + BASE_URL + "image/-1" + ")";
    ...
    var t = document.createTextNode("Login With Newauth");       // Create a text node
